...
function show(pt,e){
 ...
 tip.innerHTML=`<b>Session #${x.id} · ${x.date}</b><strong style="color:${c.css}">${isk(x.ratting_isk_hr)} ISK/hr</strong><span>${x.sites} sites · ${Math.floor(dur/60)}h ${dur%60}m</span><span>Ratting: ${isk(x.ratting_isk)}</span><span>Bounty: ${isk(x.bounty)}</span><span>ESS: ${isk(x.ess)}</span><span>Bonus/Loot: ${isk(Number(x.bonus||0)+Number(x.loot||0)+Number(x.salvage||0))}</span><span>Total: ${isk(x.total_isk)}</span>`;
